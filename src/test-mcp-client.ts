...
async function testMCPServer(): Promise<void> {
    ...
    try {
        ...
        console.log('🌐 Browser launch result:', (launchResult as ToolCallResult).content[0]?.text || 'No text content')
        ...
        console.log('🍪 Cookie result:', (cookieResult as ToolCallResult).content[0]?.text || 'No text content')
        ...
        console.log('🧭 Navigation result:', (navResult as ToolCallResult).content[0]?.text || 'No text content')
        ...
        console.log('📰 Page title:', (titleResult as ToolCallResult).content[0]?.text || 'No text content')
        ...
        try {
            await client.callTool({
                ...
                arguments: {
                    selector:
                        'textarea[data-testid="jwt-encoded-input"], .jwt-input, textarea[placeholder*="token"], textarea[placeholder*="JWT"]',
                    text: sampleToken,
                },
            })
            console.log('✅ JWT token filled successfully')
        } catch (error) {
            console.log('ℹ️ Could not fill JWT token:', (error as Error).message)
        }
        ...
        console.log('📸 Screenshot result:', (screenshotResult as ToolCallResult).content[0]?.text || 'No text content')
